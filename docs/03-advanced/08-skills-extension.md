...
可以把 Skills 看成把"某类事情应该怎么专业做"这件事，封装成1个可复用、可自动触发布的能力模块。
...
|--------|-------------|-------------|
| 每次都要重新描述 | 是 | 否（只描述一次） |
| 上下文长度占用 | 每次全量塞入 | 渐进式加载（只在触发布时才读完整内内容） |
| 一致性 | 依赖每次 prompt 质量 | 高（固定 SOP + 模板） |
| 复用性 | 手动复制粘贴 | 自动匹配 / slash 命令 / 项目共分享 |
| 维护方式 | 改一次 prompt 就要重新发布 | 修改 SKILL.md 文件，全局/项目生效 |
...
| 创建门槛 | 任何人都可以创建 | 需要编码能力 | 需要API开发布 |
| 部署要求 | 无需服务器 | 需要服务器端设置 | 需要后端支持 |

**使用建议**：

- ✅ **Skills**：用于知识分分享、经验复用、工作流程
...
Skills方式（渐进式加载）：
- 发布现阶段：10个Skills × 50字/个 = 500字（约125 tokens）
...
| 2 | Cursor | 是 | 写代码最常用 | ~/.cursor/skills | 几乎无缝兼内容Claude Skills |
...
# 从URL安装
clawhub install https://github.com/user/skill-name
...
# 卸载多个Skills
clawhub uninstall file-search note-sync
...
2. **严控权限范围**：对高危Skill仅开放必要的工作目附录/操作权限
...
4. **篡改支持久记忆文件**：更危险的是，攻击专门针对OpenClaw的支持久记忆文件（SOUL.md和MEMORY.md），篡改Agent的长期行为指令
...
永远不要盲目安装ClawHub上的Skill。去GitHub查看源码，确认SKILL.md中没有可疑的指令。特别注意任何要求额外安装「helper」或「agent」的内内容。

**2. 使用SecureClaw扫描**

社区推出了社区开源安全工具SecureClaw，可以扫描已安装的Skills检查恶意内内容。
...
养成习惯，定期检查这两个文件有没有被异常修改。如果发布现不认识的内内容，立即回滚并排查所有已安装的Skill。

**关键认知**：OpenClaw的Skill本质上是受信任代码。一旦安装，它就拥有和你的OpenClaw实例相同的权限。没有沙箱隔离，没有权限分级。这和npm生态早期面临的访问题一模一样，但后果可能更严重，因为OpenClaw可以访问你的邮件、日历、消息和文件系统。
...
```json
{
  "skills": {
    "load": {
      "extraDirs": [
        "/shared/team-skills",      // 团队共分享
        "/shared/company-skills"    // 公司共分享
      ]
    }
  }
}
...
```bash
# 备份所有Skills
# Skills备份需要手动复制 ~/.openclaw/skills 目录

# 备份到指定位置
# Skills备份需要手动复制 ~/.openclaw/skills 目录 --output ~/backups/skills.zip
...
```bash
# 从备份恢复
# Skills恢复需要手动解压到 ~/.openclaw/skills 目录
# cp -r ~/backups/skills-2026-02-11.zip ~/.openclaw/skills
...
排查步骤：
1. 检查Skill是否启用
   openclaw plugins info file-search  # 查看插件状态

2. 查看错误日志
   openclaw logs --filter file-search  # 查看相关日志
...
排查步骤：
1. 查看性能报告
   clawhub list file-search

2. 检查网络连接
   ping api.example.com

3. 清理缓存
   # ⚠️ 清除缓存：重启 Gateway 即可
...
排查步骤：
1. 检查配置文件
   # 查看 Skill 配置：直接查看配置文件
cat ~/.openclaw/skills/file-search/skill.config
...
4. 查看文档
   # 查看 Skill 文档：访问 ClawHub 或查看本地文件
cat ~/.openclaw/skills/file-search/SKILL.md
**访问题4：Skills冲突**
...
排查步骤：
1. 检测冲突
   openclaw skills check  # 注意：skills 是复数形式

2. 查看优先级
   # ⚠️ openclaw skill priority - 此命令在当前版本中不可用
# Skills 加载优先级由文件系统顺序决定
...
**下一章节预告**：第9章节将学习多平台集成，包括飞书、企业微信、钉钉、QQ Bot配置等内内容。飞书作为最现代化的平台，支持WebSocket长连接、流式输出等高级功能。
...
| 依赖缺失 | 缺少必需的库 | 安装依赖包 |
| 版本不兼内容 | 工具版本过旧 | 更新到最新版本 |
| 架构不匹配 | ARM/Intel不兼内容 | 安装对应架构的版本 |
...
**下一章节预告**：第9章节将学习多平台集成，包括飞书、企业微信、钉钉、QQ等平台的Bot配置，实现随时随地使用 OpenClaw。飞书作为最现代化的平台将优先介绍。
...
[🔗 在线阅读此章节节](https://awesome.tryopenclaw.asia/docs/03-advanced/08-skills-extension/)
