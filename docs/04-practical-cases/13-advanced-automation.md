...
内内容创作是超级个体的核心能力，但从灵感到发布布的流程很繁琐。通过自动化流程，大幅提升创作效率。

**工作流程**：
```
灵感收集 → 素材整理 → 大纲生成 → 内内容创作 → 排版优化 → 多平台发布布
```text
#### 实战配置
...
# 2. 生成大纲
echo "📋 生成大纲..."
openclaw agent --message "根据以下资料生成文章节大纲：
主题：$TOPIC
...
# 3. 创作内内容
echo "✍️ 创作内内容..."
openclaw agent --message "根据大纲创作完整文章节：
...
# 4. 优化排版
echo "🎨 优化排版..."
openclaw agent --message "优化文章节排版：
...
**场景**：每周发布布1-2篇技术博客，从选题到发布布全流程自动化。
...
# 周三：审核和优化
openclaw agent --message "审核文章节，提出优化建议：
...
# 周五：多平台发布布
openclaw agent --message "将文章节发布布到：
- 个人博客
- 掘金
- CSDN
- 知乎"
...
你：#知识点 find-skills可以发布现新的Skills
OpenClaw：已添加到知识图谱
...
```bash
openclaw agent --message "根据我的知识图谱，推荐下一步学习内内容"
...
作为开发布者，需要系统学习新技术，通过知识图谱构建完整的技术体系。
...
OpenClaw可以主动给你发布消息，实现真正的智能助手功能。

**注意**：
- ✅ 飞书支持主动发布送消息
- ⚠️ QQ目前不支持主动发布送（2026.2.6测试）
...
cat > ~/.openclaw/hooks/morning-weather.json << EOF
{
  ...
  "action": {
    "type": "sendMessage",
    "channel": "feishu",
    "message": "请发布送今日天气预报，包括温度、降水、穿衣和出行建议"
  }
}
EOF
```text
...
```
你：把这篇文章节转换成网页，生成一个可访问的链接
    [附件：文章节.md]

OpenClaw：好的，正在处理...
```text
![发布送文章节](https://upload.maynor1024.live/file/1770960879467_OpenClaw_从入门到精通指南-UpcWbWNZ2ox7R6xsszbckTqynHc.png)
...
最震撼的应用：躺在床上用自然语言指挥OpenClaw完成整个开发布流程，从代码编写到部署上线。
...
```text
![创建目附录](https://upload.maynor1024.live/file/1770960893452_OpenClaw_从入门到精通指南-Jv0GbCeYTolL34x8LdVcQohUnnc.png)
...
• 文件数量：3个（HTML/CSS/JS）
```text
![开发布游戏](https://upload.maynor1024.live/file/1770960898539_OpenClaw_从入门到精通指南-QwzJb3m24od87zxvP3Sc3yB7ny2.png)
...
**下一章节预告**：第13章节将学习创意应用探索，包括AI绘画工作流、视频脚本生成、多语言翻译、数据分析自动化和多Agent头脑风暴。
...
[🔗 在线阅读此章节节](https://awesome.tryopenclaw.asia/docs/04-practical-cases/13-advanced-automation/)
